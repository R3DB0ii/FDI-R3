...
int vec_scalar_prod(int* v1, int* v2, int size){
    int res=0;
    for(int i=0; i<size; i++){
        res += v1[i]*v2[i];
    }
...
int main() {
    ...
    int vec1[vec_dim];
    populate_vector(vec1, vec_dim);
    printf("Secondo vettore: ");
    int vec2[vec_dim];
    populate_vector(vec2, vec_dim);
    printf("Prodotto scalare tra v1 e v2: %d", vec_scalar_prod(vec1, vec2, vec_dim));
    return 0;
}
